  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-fwm-lg border border-fwm-line bg-fwm-card p-5">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {textField('formTitleVi', 'titleVi')}
        {textField('formTitleEn', 'titleEn')}
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {textField('formExcerptVi', 'excerptVi', 'textarea')}
        {textField('formExcerptEn', 'excerptEn', 'textarea')}
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {textField('formIntroVi', 'introVi', 'textarea')}
        {textField('formIntroEn', 'introEn', 'textarea')}
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {bodyField('formBodyVi', 'bodyVi')}
        {bodyField('formBodyEn', 'bodyEn')}
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {textField('formQuoteVi', 'quoteVi', 'textarea')}
        {textField('formQuoteEn', 'quoteEn', 'textarea')}
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {textField('formMistakeVi', 'mistakeVi', 'textarea')}
        {textField('formMistakeEn', 'mistakeEn', 'textarea')}
      </div>
      <div>
        <label className="mb-1.5 block font-head text-xs font-bold uppercase tracking-wide text-fwm-muted">
      ...
      {form.category === 'skill' && (
        <div className="rounded-fwm-lg border border-fwm-line bg-fwm-card-2 p-4">
          <div className="mb-1 flex items-center justify-between">
            <h3 className="font-head text-sm font-bold uppercase tracking-wide text-fwm-text">
              {t.admin.stepsHeading}
            </h3>
            ...
            {form.steps.map((step, index) => (
              <div key={index} className="rounded-fwm border border-fwm-line bg-fwm-card p-4">
                <div className="mb-3 flex items-center justify-between">
                  <span className="font-head text-xs font-bold text-fwm-accent">
                    {t.admin.stepN} {index + 1}
                  </span>
                  <button
                    type="button"
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <label className="mb-1 block text-xs text-fwm-muted">
                      {t.admin.stepTitleVi}
                    </label>
                    <input
                      required
                      value={step.titleVi}
                      onChange={updateStep(index, 'titleVi')}
                  ...
                  <div>
                    ...
                    <input
                      required
                      value={step.titleEn}
                      onChange={updateStep(index, 'titleEn')}
                      className="w-full rounded-fwm border border-fwm-line bg-fwm-card-2 px-3 py-2 text-sm text-fwm-text focus:border-fwm-accent focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      value={step.descVi}
                      onChange={updateStep(index, 'descVi')}
                      className="w-full rounded-fwm border border-fwm-line bg-fwm-card-2 px-3 py-2 text-sm text-fwm-text focus:border-fwm-accent focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      value={step.descEn}
                      onChange={updateStep(index, 'descEn')}
                      className="w-full rounded-fwm border border-fwm-line bg-fwm-card-2 px-3 py-2 text-sm text-fwm-text focus:border-fwm-accent focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      value={step.keyKind}
                      onChange={updateStep(index, 'keyKind')}
                      className="w-full rounded-fwm border border-fwm-line bg-fwm-card-2 px-3 py-2 text-sm text-fwm-text focus:border-fwm-accent focus:outline-none"
                    >
                      {KEY_KINDS.map((k) => (
                  ...
                  <div>
                    <label className="mb-1 block text-xs text-fwm-muted">
                      {t.admin.stepKeyLabel}
                    </label>
                    <div className="flex items-center gap-3">
                      <input
                        required
                        value={step.keyLabel}
                        onChange={updateStep(index, 'keyLabel')}
                        className="w-full rounded-fwm border border-fwm-line bg-fwm-card-2 px-3 py-2 text-sm text-fwm-text focus:border-fwm-accent focus:outline-none"
                      />
                      {step.keyLabel && (
                        <GamepadKey kind={step.keyKind} label={step.keyLabel} />
                      )}
                    </div>
                  </div>
                </div>
