  ...
  return (
    <header className="sticky top-0 z-40 border-b border-fwm-line bg-fwm-bg-deep/95 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
        ...
        >
          <span className="flex h-9 w-9 items-center justify-center rounded-fwm bg-fwm-accent font-head text-base font-black text-fwm-ink">
            eF
          </span>
          <span className="font-head text-lg font-extrabold text-fwm-text">
            FootballWithMe
          </span>
        </Link>
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) =>
                `rounded-fwm-pill px-3.5 py-2 font-head text-sm font-bold transition ${isActive
                  ? 'bg-fwm-pill text-fwm-accent'
                  : 'text-fwm-muted hover:text-fwm-text'
                }`
              }
            >
              {item.label}
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            ...
            <div className="hidden items-center gap-2 sm:flex">
              <Button to="/dang-nhap" variant="ghost">
                {t.nav.login}
              </Button>
