...
function CategoryDetail({ categoryId }) {
  ...
  const filtered =
    activeTag === 'all'
      ? categoryArticles
      : categoryArticles.filter((a) => a.tags.includes(activeTag));
  ...
  return (
    <div>
      <section
        className={`border-b border-fwm-line bg-gradient-to-br ${category.gradient} px-4 py-14`}
      >
        ...
        <aside>
          ...
          <div className="rounded-fwm-lg border border-fwm-line bg-fwm-card p-2">
            {popular.map((article, i) => (
              <PopularItem key={article.id} article={article} rank={i + 1} />
            ))}
          </div>
        </aside>
      </section>
